...
import LoadMyVehicles from './load-my-vehicles';
...
const MyVehicles = (): React.ReactElement => {
  const [isLoading, setIsLoading] = useState(false);
  const [valueReference, setValueReference] = useState('');
  const userData = useSelector((state: any) => state.user);
  const userProducts = userData && userData.userVehicles && userData.userVehicles.length ? userData.userVehicles : [];
  ...
  return (
    <div className='col s12 m8 MyStands'>
      <LoadMyVehicles setIsLoading={setIsLoading} />
      ...
      }
      {
        ...
        <>
          <StrongText
            fullWidth={true}
            align='left'
            text={`${userProducts.length} Vehiculo${userProducts.length === 1 ? '' : 's'} registrado${userProducts.length === 1 ? '' : 's'}`} />
              ...
              userProducts.map((i: any, index: number) => {
                return (
                  <ItemToEdit
                    ...
                    url={`/empresa/${i.relationships.stand.data.attributes.slug}/vehiculos/${i.attributes.slug}`}
