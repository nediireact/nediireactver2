...
import LoadMyRealEstates from './load-my-real-estates';
...
const MyRealEstates = (): React.ReactElement => {
  const [isLoading, setIsLoading] = useState(false);
  const [valueReference, setValueReference] = useState('');
  const userData = useSelector((state: any) => state.user);
  const userProducts = userData && userData.userRealEstates && userData.userRealEstates.length ? userData.userRealEstates : [];
  ...
  return (
    <div className='col s12 m8 MyStands'>
      <LoadMyRealEstates setIsLoading={setIsLoading} />
      ...
      }
      {
        ...
        <>
          <StrongText
            fullWidth={true}
            align='left'
            text={`${userProducts.length} Inmueble${userProducts.length === 1 ? '' : 's'} registrado${userProducts.length === 1 ? '' : 's'}`} />
              ...
              userProducts.map((i: any, index: number) => {
                return (
                  <ItemToEdit
                    ...
                    url={`/empresa/${i.relationships.stand.data.attributes.slug}/inmuebles/${i.attributes.slug}`}
