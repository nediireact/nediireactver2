...
const StandRealEstateGrid = (props: any): React.ReactElement => {
  ...
  const [classifications, setClassifications] = useState(SystemValues.getInstance().system.realEstateClassifications);
  ...
  const updateItems = ( updates: any ) => {
    ...
    APISDK.GetRealEstatesByStandId(standId, filters)
      .then((response: any) => {
        setItems(response);
      })
      .catch((error: any) => {
  ...
  useEffect(() => {
    APISDK.GetRealEstatesByStandId(standId)
      .then((response: any) => {
        setItems(response);
      })
      .catch((error: any) => {
    ...
    APISDK.GetRealEstateClassifications()
      .then(() => {
        setClassifications(SystemValues.getInstance().system.realEstateClassifications);
      })
      .catch((error: any) => console.log('Hubo un error', error));
    APISDK.GetRealEstateFeatures()
